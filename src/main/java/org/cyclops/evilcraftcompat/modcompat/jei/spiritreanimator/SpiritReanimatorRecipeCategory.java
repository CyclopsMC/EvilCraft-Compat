...
public class SpiritReanimatorRecipeCategory implements IRecipeCategory<SpiritReanimatorRecipeJEI> {
    ...
    public SpiritReanimatorRecipeCategory(IGuiHelper guiHelper) {
        ...
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(RegistryEntries.BLOCK_SPIRIT_REANIMATOR));
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, SpiritReanimatorRecipeJEI recipe, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.CATALYST, 88, 17)
    ...
    @Override
    public void draw(SpiritReanimatorRecipeJEI recipe, IRecipeSlotsView recipeSlotsView, PoseStack matrixStack, double mouseX, double mouseY) {
        ...
        fontRenderer.draw(matrixStack, duration,
                (background.getWidth() - fontRenderer.width(duration)) / 2 + 46, 50, 0xFF808080);
    }
}
